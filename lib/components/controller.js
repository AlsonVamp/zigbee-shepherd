...
class Controller extends EventEmitter {
    ...
    Q.fcall(() => {
      if (mode === 'soft' || mode === 1) {
        ...
        var steps = [
          ...
          () => {
            return this.request('SYS', 'osalNvItemInit', nvParams.znpCfgItem).delay(10).fail((err) => {
              return (err.message === 'rsp error: 9') ? null : Q.reject(err) // Success, item created and initialized
            })
          },
          () => { return this.request('SYS', 'osalNvWrite', nvParams.znpHasConfigured).delay(10) }
        ]
  ...
  request (subsys, cmdId, valObj, callback) {
    ...
    const rspHdlr = (err, rsp) => {
      ...
      if (err) {
        ...
        // unsuccessful
        deferred.reject(new Error('rsp error: ' + rsp.status)) } else { deferred.resolve(rsp) 
      }
    }
  ...
  checkNvParams (callback) {
    ...
    const steps = [
      () => {
        return this.request('SYS', 'osalNvRead', nvParams.znpHasConfigured).delay(10).then((rsp) => {
